...
import pandas as pd
from app.utils import (load_cues, extract_tasks, score_routine, score_ai_replaceability,
                       recommend_mode, cluster_label, enrich_with_ssg, adjust_scores_with_ssg, SSGTaskIndex)

st.set_page_config(page_title="AI Task Risk Analyzer (spaCy + SSG)", layout="wide")

@st.cache_resource
def get_cues():
    return load_cues("data/cue_dicts.json")

@st.cache_resource
def get_ssg_index():
    df = pd.read_csv("data/ssg_key_tasks.csv")
...
st.caption("Now with dependency-parse verb–object extraction (spaCy) and SSG key task matching.")

with st.sidebar:
    st.subheader("Filter hints (optional)")
    sector_hint = st.text_input("Sector hint")
    role_hint = st.text_input("Role hint")

tab1, tab2 = st.tabs(["Analyze JD", "Batch CSV"])

def analyze_text(jd_text: str):
...
with tab1:
    ...
    with colA:
        jd_input = st.text_area("Paste a Job Description", height=260, value=open("data/sample_job_description.txt","r",encoding="utf-8").read())
    ...
    with colB:
        ...
        if run and jd_input.strip():
            ...
            if df.empty:
                ...
                st.dataframe(df[["task_text","canonical_task","sector","role","ssg_match_sim","task_cluster","routine_score","ai_replaceability_score","mode","routine_cues","predictable_cues"]], use_container_width=True, hide_index=True)
                for _, row in df.iterrows():
                    with st.expander(f"Why: {row['task_text']}"):
                        st.write("**AI dimensions**")
                        st.json(row["dimension_breakdown"])
                csv = df.to_csv(index=False).encode("utf-8")
                st.download_button("Download CSV", data=csv, file_name="ai_task_risk_results.csv", mime="text/csv")
        ...
        if "jd_text" not in df_in.columns:
            ...
            if st.button("Run Batch"):
                ...
                if not out_rows:
                    ...
                    st.download_button("Download Batch Results", data=df_out.to_csv(index=False).encode("utf-8"), file_name="batch_ai_task_risk_results.csv", mime="text/csv")
